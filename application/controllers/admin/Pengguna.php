...
class Pengguna extends CI_Controller{
	...
	if($this->session->userdata('akses')=='1'){
		...
		if ($password2 <> $password) {
			...
			$this->m_pengguna->simpan_pengguna($nama,$username,$password,$no_hp,$level);
	...
	if($this->session->userdata('akses')=='1'){
		...
		if (empty($password) && empty($password2)) {
			$this->m_pengguna->update_pengguna_nopass($kode,$nama,$username,$no_hp,$level);
			...
			$this->m_pengguna->update_pengguna($kode,$nama,$username,$password,$no_hp,$level);
			echo $this->session->set_flashdata('msg','<label class="label label-success">Pengguna Berhasil diupdate</label>');
			redirect('admin/pengguna');
		}
	}else{
	...
	if($this->session->userdata('akses')=='1'){
		$kode=$this->input->post('kode');
		$this->m_pengguna->update_status_non($kode);
		redirect('admin/pengguna');
	}else{
        echo "Halaman tidak ditemukan";
    }
